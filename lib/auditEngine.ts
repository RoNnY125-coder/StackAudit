...
function checkRedundancy(
  ...
  const activeIds = new Set(
    tools.filter((t) => t.monthlySpend > 0).map((t) => t.id),
  );

  // These currentSpend values came from entry.monthlySpend at rule time.
  const spendByName = new Map(
    recommendations.map((r) => [r.tool.toLowerCase(), r.currentSpend]),
  );
  ...
  if (activeIds.has("anthropic") && activeIds.has("chatgpt")) {
    ...
    const cancelName =
      claudeSpend >= chatgptSpend ? "Anthropic Claude" : "ChatGPT Plus";
    const cancelId = claudeSpend >= chatgptSpend ? "anthropic" : "chatgpt";
    const cancelSpend =
      claudeSpend >= chatgptSpend ? claudeSpend : chatgptSpend;
    const cancelEntry = tools.find((t) => t.id === cancelId)!;
  ...
  if (activeIds.has("openai") && activeIds.has("anthropic")) {
    ...
    if (openaiSpend > 100) {
      const estimatedSavings = Math.round(combinedSpend * 0.3);
      extras.push({
        tool: "Dual API Spend",
        currentPlan: "—",
        currentSpend: combinedSpend,
        recommendedAction:
          "Standardize on one API provider to reduce operational complexity",
...
export function runAudit(form: FormState) {
  log.info("runAudit started", {
    teamSize: form.teamSize,
    useCase: form.useCase,
    toolCount: form.tools.length,
  });
  ...
  const perToolResults: ToolRecommendation[] = activeTools.map((entry) => {
    ...
    if (entry.usageScore <= 2) {
      log.info(
        `Low usageScore for ${entry.name} (${entry.usageScore}/10) — flagging for cancellation review`,
      );
      return {
        tool: entry.name,
    ...
    if (!ruleFn) {
      ...
      log.warn(
        `No rule found for tool id="${entry.id}" — skipping, marking optimal`,
      );
      return {
        tool: entry.name,
        currentPlan: entry.plan,
        ...
        reason:
          "No automated rule is configured for this tool. Review pricing manually.",
      };
    }

    const result = ruleFn(entry, form.useCase);
    log.info(`Audited ${entry.name}`, {
  ...
  const totalAnnualSavings = allRecommendations.reduce(
    (sum, r) => sum + r.annualSavings,
    0,
  );

  log.info("runAudit complete", {
    totalMonthlySavings,
    totalAnnualSavings,
