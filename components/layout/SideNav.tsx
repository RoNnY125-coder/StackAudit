    ...
    <aside className="fixed left-0 top-16 w-64 h-[calc(100vh-64px)] border-r border-outline-variant bg-surface-container hidden md:flex flex-col">
      ...
      <nav className="flex-1 overflow-y-auto py-4">
        <ul className="space-y-1">
          <li>
            <span className="flex px-4 py-2 text-sm bg-secondary-container text-on-secondary-container font-bold border-l-4 border-primary">
              SaaS Spend
            </span>
          </li>
        </ul>
        <p className="px-4 pt-4 text-xs text-on-surface-variant font-mono">
          More audit categories coming soon
        </p>
      </nav>
    </aside>
  )
