...
export default function SpendForm({ formProps }: SpendFormProps) {
  ...
  useEffect(() => {
    ...
    const timer1 = setTimeout(
      () => setLoadingText("Calculating savings..."),
      1000,
    );
  ...
  const [addingToCategory, setAddingToCategory] = useState<
    ToolCatalogEntry["category"] | null
  >(null);
  ...
  const handleAddCustomTool = (category: ToolCatalogEntry["category"]) => {
    ...
    addOrUpdateTool({
      ...
      monthlySpend:
        defaultPrice *
        (category === "infrastructure" || category === "llm-apis" ? 1 : seats),
      usageScore: 100,
    });
  ...
  const estMonthlySpend = Object.values(tools).reduce(
  ...
  const renderToolSection = (
    ...
    categoryTools: typeof TOOL_CATALOG,
  ) => (
    <div className="mb-8">
      ...
      {addingToCategory === category ? (
        <div className="mt-3 p-4 border border-outline-variant rounded bg-surface-container flex flex-col gap-3">
          <input
            ...
            className="bg-surface border border-outline-variant rounded p-2 text-on-surface"
          />
          <div className="flex gap-3">
            <input
              ...
              className="bg-surface border border-outline-variant rounded p-2 text-on-surface flex-1"
            />
            <input
              type="number"
              placeholder="Seats"
              value={newToolSeats}
              onChange={(e) => setNewToolSeats(e.target.value)}
              className="bg-surface border border-outline-variant rounded p-2 text-on-surface flex-1"
            />
          </div>
          <div className="flex justify-end gap-2">
            <button
              ...
              className="px-3 py-1 text-on-surface-variant hover:text-on-surface"
            >
              Cancel
            </button>
            <button
              onClick={() => handleAddCustomTool(category)}
        ...
        <button
          ...
          className="w-full mt-3 border border-dashed border-outline-variant text-on-surface-variant hover:border-outline hover:text-on-surface rounded p-3 flex items-center justify-center gap-2 transition-colors"
        >
      ...
      <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-24 self-start">
        ...
        <div className="bg-surface border border-outline-variant rounded p-4">
          <p className="text-on-surface-variant text-sm">
            <strong className="text-on-surface">{toolCount}</strong> tools added
            · Est. monthly spend:{" "}
            <strong className="text-on-surface">${estMonthlySpend}</strong>
          </p>
        </div>
