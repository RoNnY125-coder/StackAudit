...
test("Total savings sums correctly across multiple tools", () => {
  ...
  expect(result.totalAnnualSavings).toBe(
    result.recommendations.reduce(
      (sum, recommendation) => sum + recommendation.annualSavings,
      0,
    ),
  );
});
...
test("Low usage tools are flagged for cancellation review before tool rules", () => {
  ...
  expect(result.recommendations[0].recommendedAction).toBe(
    "Evaluate cancellation — usage is very low",
  );
...
test("Redundancy recommendations compute savings from cancelled tool spend", () => {
  ...
  const result = runAudit(form);
  const redundancy = result.recommendations.find((r) =>
    r.recommendedAction.includes("primary AI coding editor"),
  );
