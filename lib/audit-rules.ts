...
function optimal(
  ...
  return {
    ...
    reason:
      reason ??
      "Your current plan is cost-effective for your team size and usage pattern.",
    affiliateUrl,
  };
}
...
export function auditCursor(entry: ToolEntry): ToolRecommendation {
  ...
  if (entry.plan === "Business" && seats >= 4) {
    return optimal(
      entry,
      "Business plan is justified at 4+ seats where SSO and centralized billing save admin time.",
    );
  }
...
export function auditGithubCopilot(
  ...
  const isCodingTeam = [
    ...
    "engineering",
    "development",
  ].includes(useCase.toLowerCase().trim());
  ...
  if (entry.plan === "Business" && seats >= 10 && isCodingTeam) {
    return optimal(
      entry,
      "Copilot Business is the right call at 10+ coding seats — audit logs and policy controls pay for themselves.",
    );
  }
...
export function auditAnthropic(entry: ToolEntry): ToolRecommendation {
  ...
  if (entry.plan === "API") {
    ...
    return optimal(
      entry,
      "API spend is within a reasonable range. Monitor token volume as usage grows.",
    );
  }
  ...
  if (entry.plan === "Team" && seats >= 5) {
    return optimal(
      entry,
      "Claude Team is cost-effective at 5+ seats and includes collaborative features and admin controls.",
    );
  }
...
export function auditChatGPT(entry: ToolEntry): ToolRecommendation {
  ...
  if (entry.plan === "Enterprise" && seats >= 10) {
    return optimal(
      entry,
      "ChatGPT Enterprise is appropriate at 10+ seats with compliance requirements.",
    );
  }
...
export function auditWindsurf(entry: ToolEntry): ToolRecommendation {
  ...
  if (entry.plan === "Free") {
    return optimal(
      entry,
      "Free plan is appropriate if usage needs are being met.",
    );
  }
...
export function auditDatadog(entry: ToolEntry): ToolRecommendation {
  if (entry.plan === "Developer") {
    return optimal(
      entry,
      "Developer plan is the entry tier. Optimize by reviewing metric volume and retention settings.",
    );
  }
  ...
  return optimal(
    entry,
    "Spend is modest - ensure you are on the correct plan tier for your host count.",
  );
}
...
export function auditOpenAI(entry: ToolEntry): ToolRecommendation {
  ...
  if (
    ["Tier 2", "Tier 3", "Tier 4", "Tier 5"].includes(entry.plan) &&
    spend > 200
  ) {
  ...
  return optimal(
    entry,
    "API spend is low and well-managed. No changes needed.",
  );
}
...
export function auditNotion(entry: ToolEntry): ToolRecommendation {
  ...
  if (entry.plan === "Plus" && seats <= 3) {
    return optimal(
      entry,
      "Notion Plus is cost-effective for small teams under 4.",
    );
  }
...
export function auditLinear(entry: ToolEntry): ToolRecommendation {
  ...
  if (entry.plan === "Plus" && seats <= 10) {
    return optimal(
      entry,
      "Linear Plus is well-priced for teams under 10. No changes needed.",
    );
  }
